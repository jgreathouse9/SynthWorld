...
Section~\ref{sec:literature} situates our contribution within the existing literature on causal inference in policy evaluation. 
We then review the synthetic historical control method (SHC) in Section~\ref{sec:shc} before introducing the augmented SHC (ASHC) in Section~\ref{sec:ashc}. 
Section~\ref{sec:theory} presents our theoretical results, including finite-sample error bounds and asymptotic properties. 
We then turn to an empirical application in Section~\ref{sec:empirical}, replicating the analysis of Barcelona’s 2015 hotel moratorium from \citet{Bel2021}, a setting that allows us to compare ITS, SCM, SHC, and ASHC on equal footing. 
Finally, Section~\ref{sec:conclusion} concludes with implications for both methodological research and policy analysis.

## Literature Review {#sec:literature}

# Methodology

## The Synthetic Historical Control Method {#sec:shc}

## The Augmented Synthetic Historical Control Method {#sec:ashc}

# Theoretical Findings {#sec:theory}

# Empirical Application {#sec:empirical}

We replicate the setting of \citet{Bel2021}, who study the effects of Barcelona’s 2015 hotel moratorium using data on hotel prices from 2010 to 2017. This application provides a compelling case to benchmark ITS, SCM, SHC, and ASHC against one another.

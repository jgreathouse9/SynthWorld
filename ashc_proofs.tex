...
By the triangle inequality and the operator norm, $\\|\mathbf{C}_0^\top (\mathbf{w}^{\text{SHC}} + \Delta)\\|_2 \leq \|\mathbf{C}_0^\top \mathbf{w}^{\text{SHC}}\|_2 + \|\mathbf{C}_0^\top \Delta\|_2$. Since $\Delta = \mathbf{w}^{\text{ASHC}} - \mathbf{w}^{\text{SHC}}$, we focus on the deviation term. Dropping the non-negative data fidelity term $\frac{1}{2\lambda} \|\widetilde{\mathbf{y}} - \widetilde{\mathbf{Y}} \mathbf{w}^{\text{ASHC}}\|^2$, we get:
\[
...
\]
However, to account for the additional penalty, consider the unconstrained minimizer by taking the gradient of the objective (ignoring the simplex constraint for a moment):
\[
